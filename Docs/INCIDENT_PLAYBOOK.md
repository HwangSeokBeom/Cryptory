...
The backend gateway exposes a single public socket at `/ws/market` that normalizes all exchanges into one JSON envelope. The client is `WebSocketService` (`Cryptory/Services/WebSocketService.swift`).

- Symptoms in-app: prices stop ticking on the market/chart tabs; `publicWebSocketState` leaves `.connected`; the ViewModel derives `StreamingStatus.pollingFallback` when the state is `.disconnected`/`.failed` (`Cryptory/ViewModels/CryptoViewModel.swift:18989–19002`) and surfaces a streaming warning. Important caveat: the current client has no heartbeat/ping (`sendPing` is never called), so a half-open TCP connection can keep showing `.connected` with silently stale data — stale prices without a state change are still consistent with a WS incident.
...
- Client behavior during outage: on receive/send failure the public client schedules a reconnect with a fixed 2-second delay, no backoff or jitter (`WebSocketService.swift:448–458` via `handleFailure` `:476–487`; the URL "rotation" is a no-op with one URL). On reconnect it replays the full subscription set (`flushSubscriptions`, `:413–418`). Separately, the ViewModel activates a REST polling fallback: `updatePublicPollingIfNeeded` (`CryptoViewModel.swift:10017–10051`) polls tickers/chart data every 5 seconds (8 seconds on the kimchi tab) via `pollPublicFallback` (`:10088–10100`) while status is `.pollingFallback`.
...
- Post-incident: record duration and whether the fixed 2s reconnect caused a thundering herd on recovery (no jitter today — a known deficiency this branch's realtime refactor addresses); verify subscription replay restored all channels.
