...
2. **Transport behind a protocol** (`WebSocketTransport`): the engine never touches `URLSessionWebSocketTask`; production uses `URLSessionWebSocketTransport`, tests use a scripted transport. The transport cannot mutate UI or engine state; the engine pulls events one at a time via `receive()`, so at most one raw frame is held repository-side before decoding (Foundation/network internals are opaque and outside this bound — see REALTIME_PIPELINE.md, "Transport ingress"). The production wrapper's own state machine is additionally testable through a narrow internal driver seam (`WebSocketSocketDriver`): production delegates to `URLSessionWebSocketTask`, tests inject a controllable driver (`URLSessionTransportStateMachineTests`).
...
4. **Events via `AsyncStream<MarketStreamEvent>`** with engine-owned bounded buffering and per-kind coalescing policies keyed on the **complete market identity** (exchange + quote + symbol; tickers latest-per-identity, orderbook snapshot replacement, bounded trade batches, candle merge by interval+timestamp). The engine enforces a single live quote identity per (channel, exchange, symbol, interval), stamps quote-less wire events with the live subscription identity at decode time, and discards buffered events whose identity was replaced while pending via token validation — documented in [REALTIME_PIPELINE.md](../REALTIME_PIPELINE.md), "Market identity".
...
- Data races in the public path become compile-time impossible (actor isolation, `Sendable` events; the single `@unchecked Sendable` remaining in the new path is the documented URLSession transport boundary, whose mutable state is mutex-guarded inside a checked-`Sendable` `SharedBox`).
